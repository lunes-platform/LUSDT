...
export class LunesWalletService {
  ...
  private currentWallet: LunesWallet | null = null;
  private availableAccounts: InjectedAccountWithMeta[] = [];

  constructor(private config: LunesWalletServiceConfig) {}
  ...
  async initialize(): Promise<void> {
    try {
      ...
      throw new WalletError(
        `Failed to initialize Lunes connection: ${error instanceof Error ? error.message : 'Unknown error'}`,
   ...
   */
  async enableWalletExtensions(): Promise<InjectedAccountWithMeta[]> {
    try {
      ...
      if (extensions.length === 0) {
        throw new WalletError(
          'No wallet extensions found. Please install Polkadot.js extension.',
          'NO_EXTENSIONS'
        );
      }
      ...
      if (this.availableAccounts.length === 0) {
        throw new WalletError(
          'No accounts found in wallet extensions.',
          'NO_ACCOUNTS'
        );
      }
      ...
      throw new WalletError(
        `Failed to enable wallet extensions: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'EXTENSION_ENABLE_FAILED',
        error instanceof Error ? error : undefined
      );
    }
  }
   ...
   */
  async connect(accountAddress: string): Promise<LunesWallet> {
    if (!this.api) {
    ...
    try {
      ...
      if (!account) {
        throw new WalletError(
          `Account ${accountAddress} not found`,
          'ACCOUNT_NOT_FOUND'
        );
      }
      ...
      throw new WalletError(
        `Failed to connect to account: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'CONNECTION_FAILED',
        error instanceof Error ? error : undefined
      );
    }
  ...
  async transferNative(to: string, amount: number): Promise<TransactionResult> {
    ...
    try {
      ...
      const hash = await transfer.signAndSend(
        ...
        { signer: this.currentWallet.signer }
      );
      ...
      throw new TransactionError(
        `Failed to transfer native tokens: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'TRANSFER_FAILED',
        undefined,
        true
      );
  ...
  async subscribeToAccountChanges(
    ...
    try {
      const unsubscribe = await this.api.query.system.account(
        this.currentWallet.address,
        ({ data: balance }) => {
          const free = balance.free.toBn();
          const decimals = this.api!.registry.chainDecimals[0] || 12;
   ...
   * Desconecta da rede
   */
  async disconnect_network(): Promise<void> {
    if (this.api) {
