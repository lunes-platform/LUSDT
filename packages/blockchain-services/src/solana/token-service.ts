...
import { TokenBalance, TransactionResult, TransactionError } from '../types';
...
export class SolanaTokenService {
   ...
   */
  async transferSol(to: string, amount: number, memo?: string): Promise<TransactionResult> {
    const wallet = this.walletService.getCurrentWallet();
    if (!wallet) {
      throw new TransactionError('No wallet connected', 'NO_WALLET');
    }
   ...
   */
  async tokenAccountExists(mint: string, owner: string): Promise<boolean> {
    try {
      const mintPublicKey = new PublicKey(mint);
  ...
  async getMintInfo(mint: string) {
    try {
      ...
      throw new TransactionError(
        `Failed to get mint info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'MINT_INFO_FAILED'
      );
    }
  }
