...
export class DepositService {
   ...
   */
  async initiateDeposit(params: DepositParams): Promise<BridgeTransaction> {
    try {
      // Validate parameters
   ...
   */
  async checkDepositStatus(transactionId: string): Promise<BridgeTransaction> {
    try {
      const response = await fetch(
        `${this.config.bridgeServiceUrl}/deposits/${transactionId}/status`
      );
      ...
      throw new TransactionError(
        `Failed to check deposit status: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'STATUS_CHECK_FAILED'
      );
    }
  }
   ...
   */
  async getUserDeposits(userAddress: string): Promise<BridgeTransaction[]> {
    try {
      const response = await fetch(
        `${this.config.bridgeServiceUrl}/deposits?user=${userAddress}`
      );
   ...
   */
  async calculateDepositFee(amount: number): Promise<number> {
    try {
      const response = await fetch(
        `${this.config.bridgeServiceUrl}/deposits/fee?amount=${amount}`
      );
      ...
      throw new TransactionError(
        `Failed to calculate deposit fee: ${error instanceof Error ? error.message : 'Unknown error'}`,
   ...
   */
  private createDepositMemo(destinationAddress: string, userMemo?: string): string {
   ...
   */
  private async notifyBridgeService(transaction: BridgeTransaction): Promise<void> {
    try {
      const response = await fetch(
        `${this.config.bridgeServiceUrl}/deposits`,
        {
          method: 'POST',
          headers: {
